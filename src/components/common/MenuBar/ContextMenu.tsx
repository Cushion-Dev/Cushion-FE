    ...
    <MenuContainer>
      {type === 'global' ? (
        <>
          ...
          <Menu menuName="이용약관 확인" iconURL={ICONS.menu.externalLink}></Menu>
          <Menu menuName="로그아웃" iconURL={ICONS.menu.logout}></Menu>
          <Menu menuName="회원 탈퇴" iconURL={ICONS.menu.quit} variant="withdraw"></Menu>
        </>
      ) : (
        <>
          <Menu menuName="상대방 정보 수정" iconURL={ICONS.menu.edit}></Menu>
          <Divider src={ICONS.menu.divider} />
          <Menu menuName="운영정책 확인" iconURL={ICONS.menu.externalLink}></Menu>
        </>
      )}
    </MenuContainer>
  );
